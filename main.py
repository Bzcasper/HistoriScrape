...
from substitution_service import get_substitutions
...
@app.post(
    "/substitute", response_model=SubstituteResponse, dependencies=[Depends(rate_limit)]
)
async def substitute(request: SubstituteRequest):
    try:
        ...
        if not substitutions:
            raise HTTPException(
                status_code=404, detail="No suitable substitutions found."
            )

        response = {"status": "success", "substitutions": substitutions}
